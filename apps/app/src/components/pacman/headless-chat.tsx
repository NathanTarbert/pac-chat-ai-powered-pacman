 ...
 */
function PacManSprite({ size = 26 }: { size?: number }) {
  return (
    <div
...
function GhostIcon({ color, size = 26 }: { color: string; size?: number }) {
  return (
  ...
  return (
    <div className={`flex gap-3 ${isUser ? "flex-row-reverse" : ""} mb-4 animate-[score-pop_0.3s_ease-out]`}>
      ...
      <div className="flex-shrink-0 mt-1">
        {isUser ? (
          <div className="w-10 h-10 rounded-full bg-[#1a1a2e] border-2 border-[#ffff00] flex items-center justify-center">
            <PacManSprite size={22} />
          </div>
        ) : (
          <div className="w-10 h-10 rounded-full bg-[#1a1a2e] border-2 border-[#ff0000] flex items-center justify-center">
            <GhostIcon color={ghost.body} size={22} />
          </div>
        )}
      </div>
...
export function PacManChat() {
  ...
  return (
    <div className="relative flex flex-col h-full">
      {/* Header */}
      <div className="flex-shrink-0 px-5 py-3.5 border-b-2 border-[#2121de] bg-[#0a0a1a]">
        ...
        {visibleMessages.length === 0 && !showHITLForm ? (
          <div className="flex flex-col items-center justify-center h-full text-center gap-4">
            ...
            <div className="relative flex items-center gap-3">
              <PacManSprite size={70} />
              <div className="flex gap-2.5">
                {[0, 1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="w-3 h-3 rounded-full bg-[#ffb8ae]"
        ...
        <div className="flex gap-2 items-center">
          {visibleMessages.length > 0 && (
            <button
              ...
              className={`flex-shrink-0 w-11 h-11 rounded-lg flex items-center justify-center transition-colors cursor-pointer ${
                showSuggestions
          ...
          <button
            ...
            className="flex-shrink-0 w-11 h-11 rounded-lg bg-[#ffff00] text-black flex items-center justify-center disabled:opacity-30 hover:bg-[#ffb852] transition-colors cursor-pointer disabled:cursor-not-allowed"
